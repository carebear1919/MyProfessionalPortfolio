...
export async function sendContactEmail({ name, email, subject, message }) {
  ...
  const mailOptions = {
    ...
    subject: `Portfolio Contact: ${singleLine(subject) || '(no subject)'}`,
...
export function validateContactBody(body) {
  ...
  } else if (body.name.length > LIMITS.name) {
    errors.push(`Name must be ${LIMITS.name} characters or fewer`);
  }
  if (!isStr(body.email) || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(body.email.trim())) {
    errors.push('A valid email is required');
  } else if (body.email.length > LIMITS.email) {
    errors.push('Email is too long');
  }
  if (body.subject !== undefined && body.subject !== null && body.subject !== '') {
    if (!isStr(body.subject)) errors.push('Subject must be text');
    else if (body.subject.length > LIMITS.subject) errors.push(`Subject must be ${LIMITS.subject} characters or fewer`);
  }
  if (!isStr(body.message) || body.message.trim().length < 10) {
    errors.push('Message must be at least 10 characters');
  } else if (body.message.length > LIMITS.message) {
    errors.push(`Message must be ${LIMITS.message} characters or fewer`);
  }
  return errors;
}
